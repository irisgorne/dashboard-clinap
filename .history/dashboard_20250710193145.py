...
with col2:
    if "paciente" in locals():
        ...
        pdf_bytes = gerar_pdf_download(html_pdf)
        b64 = base64.b64encode(pdf_bytes).decode()
        href = f'<a href="data:application/pdf;base64,{b64}" download="relatorio_paciente_{paciente["ID"]}.pdf">📄 Baixar relatório individual (PDF)</a>'
        st.markdown(href, unsafe_allow_html=True)
    else:
        st.info("Selecione um paciente acima para gerar o PDF.")

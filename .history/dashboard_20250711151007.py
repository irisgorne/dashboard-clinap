...
with st.expander("📄 Exportar relatório em PDF"):
    escolha = st.radio("Escolher tipo:", ["Relatório individual", "Panorama geral", "Painel completo"])
    ...
    if escolha == "Relatório individual" and paciente is not None:
        ...
        <body>
            ...
            <h3>Dispersão Personalizada</h3>
            <p><strong>Eixo X:</strong> {x_axis} | <strong>Eixo Y:</strong> {y_axis}</p>
        ...
        pdf = gerar_pdf(html)
        b64 = base64.b64encode(pdf).decode()
        st.markdown(f'<a href="data:application/pdf;base64,{b64}" download="painel_completo.pdf">📥 Baixar Painel Completo</a>', unsafe_allow_html=True)

...
if "ID" in df_filtrado.columns:
    ...
    aciente['Escore_risco']} ({paciente['Escore_nivel']})")
        st.markdown(f"- **IMC:** {paciente['IMC']}")
        st.markdown(f"- **HbA1c:** {paciente['HbA1c']}")
        st.markdown(f"- **Calorias:** {paciente['Calorias']} kcal")ids_disponiveis = df_filtrado["ID"].dropna().unique().tolist()
    if ids_disponiveis:
        ...
        st.markdown(f"- **Escore de Risco:** {p

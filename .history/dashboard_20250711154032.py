...
with st.expander("📄 Baixar painel completo (gráficos + tabela) em PDF", expanded=False):
    ...
    # Pasta para salvar temporariamente as imagens
    os.makedirs("temp_img", exist_ok=True)

    # Salvar gráficos como imagem
    fig_disp.write_image("temp_img/grafico_disp.png", format="png", width=800, height=500)
    fig_comp.write_image("temp_img/grafico_comp.png", format="png", width=800, height=500)
    fig_hist.write_image("temp_img/grafico_hist.png", format="png", width=800, height=500)

    def gerar_html_com_imagens():
        ...
        <body>
            ...
            <img src="temp_img/grafico_disp.png">

            <h3>Gráfico 2: Comparativo</h3>
            <img src="temp_img/grafico_comp.png">

            <h3>Gráfico 3: Histograma</h3>
            <img src="temp_img/grafico_hist.png">
    ...
    def converter_html_para_pdf(html):
        result = io.BytesIO()
        pisa.CreatePDF(io.StringIO(html), dest=result, link_callback=lambda uri, rel: uri)
        return result.getvalue()

    if st.button("📥 Baixar PDF do Painel"):
        html = gerar_html_com_imagens()
